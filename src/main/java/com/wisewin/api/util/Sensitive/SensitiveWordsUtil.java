...
public class SensitiveWordsUtil {
    ...
    @Autowired
    private GradeDAO foo;
    public static Set<String> words;
    public static Map<String,String> wordMap;
    public static int minMatchTYpe = 1;      //最小匹
    // 配规则
    public static int maxMatchType = 2;      //最大匹配规则
    private static SensitiveWordsUtil sensitiveWordsUtil;
    private static boolean inst=true;
    @PostConstruct
    public void init() {
        sensitiveWordsUtil.gradeDAO = foo;
    }


    public static void start(){
        Object sensitivity = RedissonHandler.getInstance().get("sensitivity");
        if(sensitivity==null||inst){
            inst=false;
            //读取数据库
            SensitiveWordsUtil.words =gradeDAO.getSensitivity();
            System.err.println(SensitiveWordsUtil.words.size());
            addBadWordToHashMap(SensitiveWordsUtil.words);
            RedissonHandler.getInstance().set("sensitivity","sensitivity",null);
        }
    }
