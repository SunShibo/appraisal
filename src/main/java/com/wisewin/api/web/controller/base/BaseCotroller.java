...
//        RedisUtil.del(createKey(this.getLoginID(request), key)) ;
    ...
    public UserBO getLoginUser(HttpServletRequest request) {
        ...
        return (UserBO)obj;
        //return appraisalDao.getUserById(4);
    }
 ...
 /*   public UserBO getLoginUser (HttpServletRequest request ) {
     ...
     * 登录的用户存入redis
     */
    public void putRedisLoginUser(String uuid, UserBO loginUser) {
        String key=createKey(uuid, SysConstants.CURRENT_LOGIN_USER);
        Long expire=24 * 60 * 60 * 30L;
        RedissonHandler.getInstance().set(key, loginUser, expire );
    }
    ...
    public String getCookie(HttpServletRequest request, String key)  {
        ...
        for (Cookie c : cookies) {
            if (c.getName().equals(key)) {
                //System.err.println("c.getName()"+c.getName()+"c.getValue()"+c.getValue());
